...
class State:
    ...
    def frame(self):
        print('        ; cpu cycles =', self.cpu, '    offset =', self.remaining_in_frame())
        print()
        self.frame_count += 1
    ...
    def one_step(self, p1_2, routine='irq_routine_one_step', output_arg_one=True, additional_args=[]):
        self.cpu += self.rate
        self.cpu += p1_2 * 7
        self.rate = p1_2
    ...
    def two_step(self, p1, p2, routine='irq_routine_two_step', output_arg_one=True, output_arg_two=True, additional_args=[]):
        self.cpu += self.rate
        self.cpu += p1
        self.cpu += p2 * 6
        self.rate = p2
    ...
    def advance_by(self, count, last_routine_one_step='irq_routine_one_step', last_routine_two_step='irq_routine_two_step', last_routine_additional_args=[]):
        count -= (self.rate - 54)
        ...
        for step in by[-1:]:
            if isinstance(step, list):
                self.two_step(step[0], step[1], routine=last_routine_two_step, additional_args=last_routine_additional_args)
            else:
                self.one_step(step, routine=last_routine_one_step, additional_args=last_routine_additional_args)
...
def output_rows(state):
    ...
    odd = True
    for freq in [r84, r72, r72, r84, r72, r72, r84, r72, r84, r72, r72, r84, r72, r72, r84, r72, r84, r72, r72, r84, r72, r72, r84, r72, r84, r72, r72, r84, r72, r84, r72, r84]:
        if odd:
...
state2 = state.clone()
state2.advance_by(alignment_offset + 2, last_routine_one_step='irq_routine_one_step_align', last_routine_two_step='irq_routine_two_step_align', last_routine_additional_args=['lo(irq_routines_table_align_3)'])
state2.frame()

for frame in range(1, 4):
    print(f'irq_routines_table_align_{frame}:')
    state2 = state.clone()
    state2.advance_by(alignment_offset, last_routine_one_step='irq_routine_one_step_align', last_routine_two_step='irq_routine_two_step_align', last_routine_additional_args=[f'lo(irq_routines_table_align_{frame - 1})'])
    state2.frame()
