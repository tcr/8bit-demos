...
for k in results.keys():
    print(f'    byt ${results[k][0][0:2]}   ; index ${hex(int(k/2))[2:]} => {results[k][1]} = {sum(results[k][1])}')
print()
print('    align 256')
print('dmc_sync_3_4:')
for k in results.keys():
    print(f'    byt ${results[k][0][2:4]}   ; index ${hex(int(k/2))[2:]} => {results[k][1]} = {sum(results[k][1])}')
